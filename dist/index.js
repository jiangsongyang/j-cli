...
var homepage = "https://github.com/jiangsongyang/j-cli#readme";
var devDependencies = {
	"@babel/core": "^7.17.12",
	...
	"eslint-config-system-split": "^1.0.4",
...
const genTemplate = async (templateType, projectName) => {
    const root = process.cwd();
    const dirPath = node_path.resolve(__dirname, `./templates`);
...
const parseTemplateType = (templateType) => {
    switch (templateType) {
        case 'wtr':
...
const copyDir = (src, dst) => {
    node_fs.readdir(src, (err, paths) => {
        ...
        paths.forEach(path => {
            ...
            let readable;
            let writable;
            node_fs.stat(_src, (err, st) => {
                if (err) {
                    throw err;
                }
                if (st.isFile()) {
                    readable = node_fs.createReadStream(_src);
                    writable = node_fs.createWriteStream(_dst);
